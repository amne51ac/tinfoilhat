...
class UIManager {
    constructor(errorHandler) {
        ...
        if (this.errorHandler) {
            this.updateRecentTest = this.errorHandler.makeSafe(
                this, 'UIManager', 'updateRecentTest', this.updateRecentTest
            );
            this.updateRecentTestWithCurrentData = this.errorHandler.makeSafe(
                this, 'UIManager', 'updateRecentTestWithCurrentData', this.updateRecentTestWithCurrentData
            );
            this.showNotification = this.errorHandler.makeSafe(
                this, 'UIManager', 'showNotification', this.showNotification
            );
        }
    }
    ...
    startTerminalTimeUpdates() {
        try {
            ...
            if (this.errorHandler) {
                this.errorHandler.handleError('UIManager', 'startTerminalTimeUpdates', error, 'warning');
            } else {
                console.warn('Error starting terminal time updates:', error);
            }
        }
    }
    ...
    updateTerminalTime() {
        try {
            ...
            if (this.errorHandler) {
                this.errorHandler.handleError('UIManager', 'updateTerminalTime', error, 'info');
            }
        }
    }
    ...
    updateRecentTestWithCurrentData(testData) {
        try {
            if (!this.recentTestElement) {
                if (this.errorHandler) {
                    this.errorHandler.handleError('UIManager', 'updateRecentTestWithCurrentData', 
                        'Recent test element not found', 'warning');
                }
                return;
            }
            ...
            if (!testInfo) {
                if (this.errorHandler) {
                    this.errorHandler.handleError('UIManager', 'updateRecentTestWithCurrentData', 
                        'Test info element not found', 'warning');
                }
                return;
            }
            ...
            if (this.errorHandler) {
                this.errorHandler.handleError('UIManager', 'updateRecentTestWithCurrentData', error);
            } else {
                console.error('Error updating recent test display:', error);
            }
        }
    }
    ...
    updateRecentTest(testData) {
        try {
            if (!this.recentTestElement) {
                if (this.errorHandler) {
                    this.errorHandler.handleError('UIManager', 'updateRecentTest', 
                        'Recent test element not found', 'warning');
                }
                return;
            }
            ...
            if (this.errorHandler) {
                this.errorHandler.handleError('UIManager', 'updateRecentTest', error);
            } else {
                console.error('Error updating recent test display:', error);
            }
        }
    ...
    showNotification(message, type = 'info', duration = 5000) {
        try {
            ...
            // Auto-remove after duration, if duration is greater than 0
            if (duration > 0) {
                setTimeout(() => {
            ...
            if (this.errorHandler) {
                this.errorHandler.handleError('UIManager', 'showNotification', error);
            } else {
