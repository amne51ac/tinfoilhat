...
class Scanner:
    ...
    def __init__(self, num_frequencies: int = 10, min_freq: int = 900, max_freq: int = 2700, samples_per_freq: int = 1):
        """
            ...
            try:
                ...
                if "HackRF One" in result.stdout:
                    ...
                    if attempt < 3:
                        ...
                        print("WARNING: HackRF One not detected after multiple attempts. Make sure the device is connected.")
                        return False
    ...
    def _measure_power_at_frequency(self, freq_mhz: float) -> float:
        ...
        try:
            ...
            if "hackrf" in str(e).lower() or "usb" in str(e).lower():
                ...
                if not self.refresh_hackrf():
                    raise RuntimeError(f"HackRF connection lost during measurement at {freq_mhz} MHz: {str(e)}")
            raise
    ...
    def get_baseline_readings(self) -> List[float]:
        ...
        if not self.hackrf_available:
            ...
            if not self.refresh_hackrf():
                raise RuntimeError("HackRF device is not available and reconnection failed. Cannot perform measurements.")
        ...
        for freq in self.frequencies:
            ...
            for i in range(self.samples_per_freq):
                try:
                    ...
                    print(f"Error measuring at {freq} MHz (sample {i+1}/{self.samples_per_freq}): {str(e)}")
    ...
    def calculate_attenuation(self, baseline: List[float], hat_readings: List[float]) -> List[float]:
        ...
        for freq, base, hat in zip(self.frequencies, baseline, hat_readings):
            ...
            if att < 0:
                ...
                print(f"Warning: Negative attenuation detected at {freq} MHz. Likely measurement variation.")
                att = 0.1  # Set to a minimal positive value
            
            # Apply frequency-dependent adjustment (tinfoil hats typically perform better at higher frequencies)
            # Calculate frequency factor (normalized to 0-1 range)
            freq_factor = (freq - self.min_freq) / (self.max_freq - self.min_freq) if self.max_freq > self.min_freq else 0
            ...
            print(f"At {freq} MHz: Baseline {base:.2f} dBm, Hat {hat:.2f} dBm, Attenuation {att:.1f} dB")
        ...
        if max(attenuation) < 2.0:
            ...
            print(f"\nThe hat performs best at {peak_freq} MHz with {peak_attenuation:.1f} dB attenuation")
