...
def measure_frequency():
    ...
    try:
        ...
        attenuation = None
        if (measurement_type == "hat" and 
            "BASELINE_DATA" in current_app.config and 
            str(freq_hz) in current_app.config["BASELINE_DATA"]):
    ...
    try:
        ...
        for i, freq in enumerate(scanner.frequencies):
            db.execute(
                ...
                VALUES (?, ?, ?, ?, ?)
                """,
                (test_result_id, float(freq), float(baseline_readings[i]), 
                 float(hat_readings[i]), float(attenuation_data[i]))
            )
